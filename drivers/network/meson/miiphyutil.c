...
// we have no malloc!
struct legacy_mii_dev myldev;
struct mii_dev mybus;
...
static int legacy_miiphy_read(struct mii_dev *bus, int addr, int devad, int reg)
{
    unsigned short val;
    int ret;
    volatile struct legacy_mii_dev *ldev = bus->priv;
                                  ...
                                  unsigned char reg, unsigned short value))
{
    ...
    struct legacy_mii_dev *ldev = &myldev;
...
int miiphy_reset(const char *devname, unsigned char addr)
{
    ...
    if ((reg & 0x8000) == 0) {
        ...
        microkit_dbg_puts("PHY reset timed out\n");
        return -1;
    }
    return 0;
}
...
int miiphy_duplex(const char *devname, unsigned char addr)
{
    ...
    if (bmcr & BMCR_ANENABLE) {
        /* Get auto-negotiation results. */
        if (miiphy_read(devname, addr, MII_LPA, &anlpar)) {
            microkit_dbg_puts("PHY AN duplex");
            goto miiphy_read_failed;
        }
...
int miiphy_link(const char *devname, unsigned char addr)
{
    ...
    if (miiphy_read(devname, addr, MII_BMSR, &reg)) {
        microkit_dbg_puts("MII_BMSR read failed, assuming no link\n");
        return 0;
    }
